class Solution:
    ...
    def get_k_continuous_largest(self, total, l):
        if not l:
            return 0
        if total >= len(l) / 2 + 1:
            return sum([i for i in l if i > 0])
        # f[n+2][k] = max(f[n][k], f[i][k-1] + max[i+1, n+2])
        f = dict([(i, dict([(j, 0) for j in range(0, total+1)])) for i in range(0, len(l)/2 + 2)])
        f[1][1] = l[0]
        print l, len(l)
        try:
            for k in range(1, total+1):
                #: nth postive number
                for n in range(1, len(l)/2 + 2):
                    if n == 1:
                        f[n][k] = l[0]
                        continue
                    ...
                    for i in range(n-1, k-2, -1):
                        if i == n-1:
                            ...
                            tmp += l[2 * i] + l[2 * i + 1]
                        f[n][k] = max(f[n][k], f[i][k-1] + tmp)
        except (KeyError, IndexError):
            print l
            print f
            print n, k, i
            raise
        return f[len(l)/2 + 1][total]
